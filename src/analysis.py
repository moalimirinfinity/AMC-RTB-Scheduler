...
def solve_response_time_equation(wcet: int, interferences: List[Tuple[int, int]], deadline: int) -> int:
    ...
    while True:
        interference_sum = 0
        for period, hp_wcet in interferences:
            # --- FIX: Add safety check to prevent division by zero ---
            if period > 0:
                interference_sum += math.ceil(response_time / period) * hp_wcet
...
def analyze_transition_schedulability_rtb(task_set: List[Task], r_lo_values: Dict[int, int]) -> bool:
    ...
    for task in sorted(hi_crit_tasks, key=lambda t: t.priority):
        ...
        fixed_lo_interference = 0
        for hp in hp_tasks:
            if hp.criticality == 'LO' and hp.period > 0:
                fixed_lo_interference += math.ceil(r_i_lo / hp.period) * hp.wcet_lo
